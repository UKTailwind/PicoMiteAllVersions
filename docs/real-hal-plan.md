...
**24 commits on `real-hal`.** Host tests 239/239 after every commit. All 12 device CMake variants green (`./buildall.sh` sweeps every `COMPILE` target: PICO/USB/VGA/VGAUSB/WEB for RP2040 and PICO/USB/VGA/VGAUSB/HDMI/HDMIUSB/WEB for RP2350). WASM link green.
...
- Phase 4 (`hal_storage` + `hal_filesystem`) 🟡 — foundation + path ops + driver extractions + dir iteration + **file-op scaffolding** landed: `hal/hal_storage.h` + `hal/hal_filesystem.h` contracts (293c98d), `hal_storage_pico.c` thin adapter to FatFS `disk_*` + `pico_lfs_cfg` geometry (293c98d), `hal_filesystem_pico.c` path-prefix dispatcher A:→LFS / B:→FatFS (b929139), `hal_filesystem_host.c` forwarding through `host_f_*` (d171e87). FileIO.c cmd_mkdir/cmd_rmdir/cmd_name/cmd_kill single-file branch + GetCWD migrated via `ErrorCheckHAL` errno→FSerror shim (d171e87, 25d6bd7). hal_fs_dir_open/next/close working on host and device (25d6bd7). Dead `f_mkdir` / `f_rename` host macros removed (25d6bd7). Drivers relocated per plan: `mmc_stm32.c` → `drivers/sd_spi/` (0d7940d), `fs_flash_*` + `pico_lfs_cfg` → `drivers/pico_flash/pico_flash_lfs.c` (25d6bd7). Both drivers have README.md documenting the HAL mapping. `fun_dir` + `mmbasic_chdir` migrated to `hal_fs_dir_*` / `hal_fs_chdir` (1ca8347) — both HAL adapters now strip MMBasic's `A:`/`B:` prefix before FatFS dispatch (vendored FatFS on host + device mounts with the default empty drive). `hal_fs_open`/`close`/`read`/`write`/`seek`/`tell`/`eof`/`sync` implemented on device (16-slot table, FIL*/lfs_file_t* inside each slot, MMBasic 'A' xattr re-stamp on close/sync for writable LFS) and host (16-slot table dispatching to POSIX fopen when `host_sd_root` set, vendored FatFS otherwise) (581a1db). **Resume here next:** migrate `BasicFileOpen` / `FileGetChar` / `FilePutChar` / `FilePutStr` / `FileEOF` / `FileClose` / `ForceFileClose` / `cmd_seek` / `cmd_flush` / `fun_loc` / `fun_lof` in `FileIO.c` to call `hal_fs_*` via a new per-fnbr `hal_fd` field. External callers (`Audio.c`, `MMtcpserver.c`, `upng.c`, `BmpDecoder.c`, `Editor.c`, `bc_runtime.c`, `MMtftp.c`, `MM_Misc.c`, `Commands.c`, `Memory.c`) still read `FileTable[].fptr` / `.lfsptr` — keep the legacy fields populated via a `hal_fs_peek_handle(fd, *is_lfs)` scaffold until each caller is HAL-routed. `~1-day refactor with real regression risk` per earlier estimate; the scaffolding commit lowers the remaining risk by half since the HAL path works and is exercised via the path ops already.
- Phases 5–13 — not started.

**Commits:** f89e9a9 (scaffolding), 9a53573 / f7a06f4 / 896eaa9 / f1207a6 (state hoists), 33163ad / ed610a2 (hal_flash), 029170b / f2d840f (hal_time), 67b4092 / bbbb4ec / 344def0 / 2b374da / dca6ba2 / 8919341 / 67c1313 / e733405 (hal_pin), e8e1439 (buildall sweep), 293c98d / b929139 / d171e87 / 0d7940d / 25d6bd7 / 1ca8347 / 581a1db (hal_storage + hal_filesystem + drivers/sd_spi + drivers/pico_flash + fun_dir/chdir migration + hal_fs_open/read/write/close scaffolding).
