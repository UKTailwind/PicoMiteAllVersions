...
**20 commits on `real-hal`.** Host tests 239/239 after every commit. All 12 device CMake variants green (`./buildall.sh` sweeps every `COMPILE` target: PICO/USB/VGA/VGAUSB/WEB for RP2040 and PICO/USB/VGA/VGAUSB/HDMI/HDMIUSB/WEB for RP2350). WASM link green.
...
- Phase 4 (`hal_storage` + `hal_filesystem`) 🟡 — foundation + first migrations landed: `hal/hal_storage.h` (block-level) + `hal/hal_filesystem.h` (POSIX) contracts (293c98d), `ports/pico_sdk_common/hal_storage_pico.c` thin adapter to FatFS `disk_*` + `pico_lfs_cfg` geometry (293c98d), `host/hal_storage_host.c` null impl (293c98d), `ports/pico_sdk_common/hal_filesystem_pico.c` path-prefix dispatcher (A:→LFS, B:→FatFS) (b929139), `host/hal_filesystem_host.c` forwarding through `host_f_*` (d171e87). FileIO.c `cmd_mkdir`/`cmd_rmdir`/`cmd_name`/`cmd_kill` single-file branch migrated via new `ErrorCheckHAL` errno→FSerror shim (d171e87). **Resume here next:** migrate file-table ops (BasicFileOpen / FileGetChar / FilePutChar / FileClose / FileEOF / fun_loc / fun_lof / cmd_seek / cmd_flush); hal_fs_open/close/read/write currently return -ENOSYS on device. `mmbasic_chdir` (complex relative-path resolver) + the `fun_dir` FILES command stay on legacy dispatch until opendir/readdir land. Driver extractions (`mmc_stm32.c` → `drivers/sd_spi/`, `fs_flash_*` → `drivers/pico_flash/`) are a separate structural step.
- Phases 5–13 — not started.

**Commits:** f89e9a9 (scaffolding), 9a53573 / f7a06f4 / 896eaa9 / f1207a6 (state hoists), 33163ad / ed610a2 (hal_flash), 029170b / f2d840f (hal_time), 67b4092 / bbbb4ec / 344def0 / 2b374da / dca6ba2 / 8919341 / 67c1313 / e733405 (hal_pin), e8e1439 (buildall sweep), 293c98d / b929139 / d171e87 (hal_storage + hal_filesystem).
