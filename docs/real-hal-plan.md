...
**18 commits on `real-hal`.** Host tests 239/239 after every commit. All 12 device CMake variants green (`./buildall.sh` sweeps every `COMPILE` target: PICO/USB/VGA/VGAUSB/WEB for RP2040 and PICO/USB/VGA/VGAUSB/HDMI/HDMIUSB/WEB for RP2350). WASM link green.
...
- Phase 4 (`hal_storage` + `hal_filesystem`) 🟡 — foundation landed (293c98d): `hal/hal_storage.h` (block-level) + `hal/hal_filesystem.h` (POSIX) contracts, `ports/pico_sdk_common/hal_storage_pico.c` thin adapter to FatFS `disk_*` + `pico_lfs_cfg` geometry, `host/hal_filesystem_host.c` full libc/POSIX impl, `host/hal_storage_host.c` null impl. No FileIO.c caller migration yet — impls are dead code on host until callers switch. **Resume here next:** migrate FileIO.c call sites (BasicFileOpen/FileGetChar/FilePutChar/FileClose/FileEOF/fun_loc/fun_lof/cmd_seek/cmd_flush on host, and the FatFS/LFS dispatcher on device) through the new HAL; extract `mmc_stm32.c` into `drivers/sd_spi/` and the `fs_flash_*` + `pico_lfs_cfg` block backing into `drivers/pico_flash/`.
- Phases 5–13 — not started.

**Commits:** f89e9a9 (scaffolding), 9a53573 / f7a06f4 / 896eaa9 / f1207a6 (state hoists), 33163ad / ed610a2 (hal_flash), 029170b / f2d840f (hal_time), 67b4092 / bbbb4ec / 344def0 / 2b374da / dca6ba2 / 8919341 / 67c1313 / e733405 (hal_pin), e8e1439 (buildall sweep), 293c98d (hal_storage + hal_filesystem foundation).
