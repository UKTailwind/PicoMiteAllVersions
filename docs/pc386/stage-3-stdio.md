...
- **3c.1 ⏳ — Freestanding libc + MMBasic core in the link.** Bigger than the one-line summary suggests. i686-elf-gcc ships no `<stdio.h>`, `<string.h>`, `<math.h>`, `<setjmp.h>`, `<errno.h>`, `<complex.h>`, `<time.h>`, `<ctype.h>`, `<stdlib.h>` — every one of these has to exist on the include path before any MMBasic core file compiles. Three buckets:
    1. **Hand-rolled in `ports/pc386/include_shim/` + `pc386_freestanding_libc.c`:** the simple stuff — string ops (memcpy/memset/strchr/strlen/strcpy/strcmp/strcat/strncmp/strncpy/strncat/memcmp/memmove), ctype (isdigit family — single-byte table lookups), errno (just `extern int errno`), stdlib (atoi/atof/abs/labs/llabs/abs64/qsort + a malloc/free wrapper over `bc_alloc`).
    2. **Vendor from openlibm + mpaland/printf:** `<math.h>` (sin, cos, tan, asin, acos, atan, atan2, log, log10, exp, sqrt, pow, floor, ceil, fabs, fmod, modf, frexp, ldexp, pow10) is non-trivial — vendor the relevant openlibm sources rather than hand-rolling. `<stdio.h>` printf/sprintf/snprintf/vsnprintf comes from a minimal `printf.c` (mpaland's ~700-LOC public-domain impl is the obvious pick).
    3. **i686-specific inline-asm:** `setjmp.h`'s `setjmp`/`longjmp` is ~30 lines of hand-written x86-32 inline asm.
   
    Estimated ~1500 LOC vendored + ~400 LOC hand-rolled. The mmbasic_stdio Makefile's `CORE_SRCS` + `BC_SRCS` (~30 files) come into the pc386 link only after this layer is solid. The output of 3c.1 is a clean compile of `Operators.c` (smallest core file) as the canary.
