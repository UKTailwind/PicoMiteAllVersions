...
| [2 — disk + FS + Limine boot](pc386/stage-2-disk.md) | ✅ | ATA-PIO + FatFs over IDE. A: (8 MB partitioned FAT12 boot disk, Limine MBR) + C: (32 MB FAT16 superfloppy). Boot off A: via Limine multiboot1; kernel mounts both as DOS-style "A:"/"C:". Sub-stages 2a (ATA-PIO), 2b (FatFs), 2c (drive letters via FF_VOLUME_STRS injection), 2d (Limine bios-install). |
| [3 — stdio port](pc386/stage-3-stdio.md) | ✅ | MMBasic interpreter runs over COM1: full REPL with banner / line editing, FILES / LOAD / LIST / RUN of HELLO.BAS + FIZZBUZZ.BAS on B: (FatFs vol 0) or A: (alias), error-stub corpus for unsupported PLAY / FRAMEBUFFER / SETPIN. 12-test Python harness (`ports/pc386/tests/repl_expect.py`) green; host_native still 243/243. mmbasic.elf 3 MB, freestanding under i686-elf-gcc. |
