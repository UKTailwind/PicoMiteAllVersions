...
// --- Soft-keyboard support for touch devices ----------------------------
// Tapping the canvas focuses a hidden <input> to summon the OS keyboard.
// Hardware keyboards keep working through the window-level keydown handler
// above; the input only matters when the user is on a touch device where
// keydown/keyup are unreliable for soft keyboards.
const kbdInput = document.getElementById("kbd-helper");

function focusKbdInput() {
    if (!kbdInput) return;
    // Keep value empty so each soft-keyboard character fires a fresh
    // `input` event with a single-char value.
    kbdInput.value = "";
    kbdInput.focus({ preventScroll: true });
}

// Use pointerdown so the focus call happens inside the user-gesture
// handler — browsers (especially iOS Safari) only honor programmatic
// focus that raises the keyboard if it occurs synchronously during a
// trusted gesture.
canvas.addEventListener("pointerdown", focusKbdInput);
// Some older mobile browsers don't dispatch pointer events for stylus /
// passive touches; touchend is a reliable fallback.
...
if (kbdInput) {
    // Soft keyboards on mobile often skip keydown/keyup and only fire
    // `input` (and `beforeinput` for edits). Translate those to PicoMite
    // byte codes and forward through the same sendKey path.
    ...
    // Stop typed characters from drifting into the input on desktop (the
    // window keydown handler already preventDefaults, but belt-and-braces
    // for IMEs and composition events).
...
// --- Viewport shift when the soft keyboard appears ----------------------
// On phones, the soft keyboard overlays the bottom of the page without
// resizing window.innerHeight. visualViewport tracks the *visible* area,
// so we resize the body to match it and pin content to the top — the
// canvas stays above the keyboard instead of being hidden behind it.
...
function syncViewport() {
    ...
    if (focused && occluded > 80) {
        ...
        // Scroll the canvas into view in case the page got pushed.
