...
static void memcopy(uint8_t* d, const uint8_t* s, unsigned long size )
{
...
static void worknote(CombinedPtrT<note> nptr, channel * cptr,char t,modcontext * mod )
{
	...
	note n = *nptr;
	sample = (n.sampperiod & 0xF0) | (n.sampeffect >> 4);
	period = ((n.sampperiod & 0xF) << 8) | n.period;
	effect = ((n.sampeffect & 0xF) << 8) | n.effect;
	effect_op = n.sampeffect & 0xF;
	effect_param = n.effect;
...
static void workeffect( modcontext * modctx,  CombinedPtrT<note> nptr, channel * cptr )
{
	doFunk(cptr);
...
static void memcopy2(uint8_t* dst, CombinedPtr src, size_t sz) {
	for (int i = 0; i < sz; ++i) {
		*dst++ = *src++;
	}
}
...
int hxcmod_load(modcontext * modctx, CombinedPtr mod_data, int mod_data_size)
{
	...
	sample *sptr;
	CombinedPtr modmemory, endmodmemory;
	...
	if( modmemory )
	{
		if( modctx )
		{
			...
			if( !modctx->number_of_channels )
			{
				...
				memcopy((uint8_t*)&(modctx->song.signature), (uint8_t*)"M.K.", 4);
				memcopy((uint8_t*)&(modctx->song.length), (uint8_t*)&(modctx->song.samples[15]), 130);
			...
			if( modctx->number_of_channels > NUMMAXCHANNELS ) {
				error("Too much channels: % in $", modctx->number_of_channels, modctx->song.title);
				return 0; // Too much channels ! - Increase/define HXCMOD_MAXCHANNELS !
			}
			if( modmemory >= endmodmemory ) {
				error("End passed ? - Probably a bad file !");
			...
			for (i = 0; i < 128; i++)
			{
				...
				{
					modctx->patterndata[max] = modmemory;
	...
	if( modctx && outbuffer )
	{
		if(modctx->mod_loaded)
		{
			...
			if( trkbuf )
			{
				...
				memcopy((uint8_t*)trkbuf->name, (uint8_t*)modctx->song.title, sizeof(modctx->song.title));

				for(i=0;i<31;i++)
				{
					memcopy((uint8_t*)trkbuf->instruments[i].name, (uint8_t*)modctx->song.samples[i].name, sizeof(trkbuf->instruments[i].name));
				}
			}
			...
			for (i = 0; i < nbsample; i++)
			{
				...
				if( modctx->patternticks++ > modctx->patternticksaim )
				{
					if( !modctx->patterndelay )
					{
						...
						{
							worknote(nptr + c, (channel*)(cptr+c), (char)(c+1), modctx);
						}
