...
void *bc_alloc(size_t size) { (void)size; return NULL; }
void bc_free(void *ptr) { (void)ptr; }
void bc_alloc_reset(void) {}
void *bc_compile_alloc(size_t size) { (void)size; return NULL; }
void bc_compile_free(void *ptr) { (void)ptr; }
...
void bc_run_diag_note_before_clear(unsigned old_used, unsigned old_free, unsigned old_contig) {
    (void)old_used; (void)old_free; (void)old_contig;
}
...
void bc_run_diag_note_vm_stage(BCRunVmStage stage, unsigned vm_used, unsigned vm_hw,
    ...
    (void)stage; (void)vm_used; (void)vm_hw;
    (void)c_used; (void)c_free; (void)r_lim; (void)cap;
}
void bc_run_diag_dump(const char *reason) { (void)reason; }
