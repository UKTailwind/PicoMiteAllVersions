'PicoVaders — FASTGFX variant, 60 fps
'-------------------------------------
'
...
'
' Modified to use FASTGFX double-buffering for flicker-free sprites,
' with velocity-driven ship movement and an on-screen FPS counter.
'
'-----------------Init----------------
...
BX=0:BY=0:BA=0
plx=103: pvx=0
' Paddle physics — accel-per-keystroke, decay-per-frame, max velocity.
...
tn=1
Intro
' Intro runs without FASTGFX (lots of iPauses; would need a SWAP per
' step).  Activate double-buffering now for the gameplay loop so the
' erase-then-redraw passes for sprites never reach the user's eye.
FASTGFX CREATE
FASTGFX FPS 60
Level%=1
Score%=0
...
tic=1
bn%=1
fps_count=0: fps_t0=Timer: fps_show=0
...
Do
  ps=5 :'Timing
  move_single
  ' Move_Player erases the previous-frame ship position; Draw_Player
  ' then paints at the new position.  Under FASTGFX the original
  ' "Draw then Move (which erased)" order wiped the ship before SWAP
  ' on every frame that the int position changed.
  Move_Player
  Draw_Player
  Draw_Bullet
  Draw_Bomb
  ...
  ' Frame-rate counter — updates the on-screen number once per second.
  ' Drawn before SWAP so it lands on the about-to-be-presented frame.
  Inc fps_count
  If Timer - fps_t0 >= 1000 Then
    fps_show = fps_count
    fps_count = 0
    fps_t0 = Timer
  EndIf
  Print @(244,0);"FPS";fps_show;"  "
  ' Present the back buffer; SYNC paces to FASTGFX FPS.
  FASTGFX SWAP
  FASTGFX SYNC
Loop
If plHit% Then
...
GmOV:
If gameover%=1 Then
   ' Drop back to direct draw for the flashing "GAME OVER" banner —
   ' iPause has no SWAP, so under FASTGFX the user would see a frozen
   ' last frame for 12 seconds.  CLOSE flushes the back buffer to the
   ' front first so we don't lose the final game state.
   FASTGFX CLOSE
...
If A_Bomb%(f,3)=1 Then
  ...
  If BA Then
    ...
    Select Case A_Bomb%(f,2)
     Case by-4 To by
      ba=0: A_Bomb%(f,3)=0
      Inc Bombs_out%,-1
      ' Erase the ship bullet on the back buffer BEFORE expl SWAPs,
      ' otherwise it sits on top of the explosion for the freeze.
 ...
 EndIf
' (Removed stray `Line 50+bx,by,...,0` that erased the ship bullet on
'  every bomb iteration — invisible to the player without FASTGFX
'  because the next Draw_Bullet re-drew it, but with FASTGFX it wipes
'  the bullet on the back buffer before SWAP.)
      ...
      Select Case blx
        Case ax+1 To ax+13
          Select Case bly
            Case ay To ay+7
              collition1=1
              ' Erase the ship bullet on the back buffer BEFORE expl
              ' SWAPs, otherwise it sits on top of the explosion for
              ' the freeze.
              Line 50+blx,bly,50+blx,bly+4,,0
              expl ax+50,ay,1
              aliens(f,4)=0
...
Sub expl ex,ey,snd
 ' Draw the explosion sprite + present so it's visible while the sound
 ' plays.  Box-erase + final SWAP afterwards so the cleared scene
 ' appears immediately, not on the next main-loop tick.
 GUI BITMAP ex,ey,xpl$,16,8,1,gelb,0
 Draw_Ufo
 FASTGFX SWAP
 If snd=1 Then
   For nse=1 To 75
    Play tone noise%(nse),noise%(nse)
    Pause 1
   Next nse
   Play tone 0,0,1
 Else
...
Sub Move_Player
  ...
  '
  ' Order matters under FASTGFX: erase the PREVIOUS frame's ship from
  ' the back buffer FIRST, then update plx, then let Draw_Player paint
  ' the new position.  Drawing then erasing in the same frame wipes
  ' the ship out before SWAP.
  Local fire%
  Box 50+Int(plx),214,16,10,,0,0
  Do
    k$=Inkey$
    If k$="" Then Exit Do
    If Asc(k$)=27 Then FASTGFX CLOSE: End
    If Asc(k$)=130 Then
      pvx = pvx - PAD_ACCEL
...
Sub Expl_Player
  'show Player Explosion — each animation frame needs its own SWAP so
  'the sequence is visible while FASTGFX is active.
  For f=1 To 3
    GUI BITMAP 50+plx,214,ply$(2),16,8,1,grn,0
    FASTGFX SWAP
    For nse=1 To 100:Play tone noise%(nse),noise%(nse):Pause 1:Next nse
    GUI BITMAP 50+plx,214,ply$(3),16,8,1,grn,0
    FASTGFX SWAP
    For nse=100 To 200:Play tone noise%(nse),noise%(nse):Pause 1:Next nse
  Next f
  For nse=1 To 200:Play tone noise%(nse),noise%(nse):Pause 1:Next nse
  Play tone 0,0,1
  Pause 500
