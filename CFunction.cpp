...
long long int MIPS16 CallCFunction(CombinedPtr CmdPtr, CombinedPtr ArgList, CombinedPtr DefP, CombinedPtr CallersLinePtr) {
            ...
            if(isnamestart((uint8_t)*argv[i]) && (*skipvar(argv[i], false) == 0 || *skipvar(argv[i], false) == ')') && !(FindSubFun(argv[i], 1) >= 0 && strchr((const char *)argv[i], '(') != NULL)) {
                ...
                switch(typ[i/2]) {
                    ...
                    case T_STR: arg[i/2] = GetTempMemory(STRINGSIZE);
                                Mstrcpy((uint8_t*)arg[i/2], getstring(argv[i]));
                                break;
