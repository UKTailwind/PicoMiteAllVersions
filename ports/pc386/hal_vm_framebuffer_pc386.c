...
{ (void)fast; error("FRAMEBUFFER not available on mode 13h display"); }

void hal_vm_framebuffer_layer(int has_colour, int colour)
{ (void)has_colour; (void)colour; error("FRAMEBUFFER not available on mode 13h display"); }

void hal_vm_framebuffer_write(char which)
{ (void)which; error("FRAMEBUFFER not available on mode 13h display"); }
