...
int vsnprintf(char * out, size_t n, const char * fmt, va_list ap) {
    ...
    while (*fmt) {
        ...
        case '%':
            ...
            break;
        case 'c':
            pc386_emit(&p, end, (char)va_arg(ap, int));
            break;
        case 's': {
...
int putchar(int c) {
    extern char SerialConsolePutC(char, int);
    return SerialConsolePutC((char)c, 0);
}
int puts(const char * s) {
    extern void SSPrintString(char *);
    SSPrintString((char *)s);
    putchar('\n');
    return 0;
}
int fgetc(FILE * fp) {
...
char * strncat(char * d, const char * s, size_t n) {
    ...
    if (n == (size_t)-1)
        *d = '\0'; /* loop wrote terminator already */
    else
        *d = '\0';
    return r;
}
...
double strtod(const char * s, char ** end) {
    ...
    if (any && (*p == 'e' || *p == 'E')) {
        ...
        if (exp_any)
            v *= pc386_pow10_int(exp_neg ? -exp_val : exp_val);
        else
            p = exp_start;
    }
...
int rand(void) {
    return (int)(hal_random_u32() & 0x7FFFFFFFu);
}
void srand(unsigned int seed) {
    pc386_rand_state = seed ? seed : 1;
}
...
size_t strftime(char * s, size_t n, const char * fmt, const struct tm * tm) {
    ...
    while (*fmt && pos + 1 < n) {
        ...
        switch (*fmt) {
        case 'Y':
            pos = pc386_strftime_append_num(s, n, pos, tm->tm_year + 1900, 4);
            break;
        case 'y':
            pos = pc386_strftime_append_num(s, n, pos, (tm->tm_year + 1900) % 100, 2);
            break;
        case 'm':
            pos = pc386_strftime_append_num(s, n, pos, tm->tm_mon + 1, 2);
            break;
        case 'd':
            pos = pc386_strftime_append_num(s, n, pos, tm->tm_mday, 2);
            break;
        case 'H':
            pos = pc386_strftime_append_num(s, n, pos, tm->tm_hour, 2);
            break;
        case 'M':
            pos = pc386_strftime_append_num(s, n, pos, tm->tm_min, 2);
            break;
        case 'S':
            pos = pc386_strftime_append_num(s, n, pos, tm->tm_sec, 2);
            break;
        case 'j':
            pos = pc386_strftime_append_num(s, n, pos, tm->tm_yday + 1, 3);
            break;
        case 'I': {
            int h = tm->tm_hour % 12;
            if (h == 0) h = 12;
        ...
        case 'p':
            pos = pc386_strftime_append(s, n, pos, tm->tm_hour < 12 ? "AM" : "PM");
            break;
        case 'A':
...
char * asctime(const struct tm * tm) {
    (void)tm;
    pc386_panic("asctime not impl");
}
char * ctime(const time_t * t) {
    (void)t;
    pc386_panic("ctime not impl");
}
...
double atan(double x) {
    ...
    double x2 = x * x;
    double r = x * (1.0 - x2 / 3.0 + (x2 * x2) / 5.0 - (x2 * x2 * x2) / 7.0 + (x2 * x2 * x2 * x2) / 9.0 - (x2 * x2 * x2 * x2 * x2) / 11.0 + (x2 * x2 * x2 * x2 * x2 * x2) / 13.0);
...
#define CPLX_STUB1(rt, name)                                         \
    rt name(float complex z) {                                       \
        (void)z;                                                     \
        pc386_panic(#name " not yet implemented (vendor openlibm)"); \
    }
#define CPLX_STUB1D(rt, name)                                        \
    rt name(double complex z) {                                      \
        (void)z;                                                     \
        pc386_panic(#name " not yet implemented (vendor openlibm)"); \
    }
#define CPLX_STUB2(rt, name)                                         \
    rt name(float complex z, float complex w) {                      \
        (void)z;                                                     \
        (void)w;                                                     \
        pc386_panic(#name " not yet implemented (vendor openlibm)"); \
    }
#define CPLX_STUB2D(rt, name)                                        \
    rt name(double complex z, double complex w) {                    \
        (void)z;                                                     \
        (void)w;                                                     \
        pc386_panic(#name " not yet implemented (vendor openlibm)"); \
    }
