 ...
 * unconditionally. The OnPS2GOSUB interrupt path checks PS2int and never
...
uint8_t APP_MapKeyToUsage(uint8_t keyCode, int modifier) {
    if (keyCode == USB_HID_KEYBOARD_KEYPAD_KEYBOARD_CAPS_LOCK || keyCode == USB_HID_KEYBOARD_KEYPAD_KEYBOARD_SCROLL_LOCK || keyCode == USB_HID_KEYBOARD_KEYPAD_KEYPAD_NUM_LOCK_AND_CLEAR) return 0;
...
void USR_KEYBRD_ProcessData(uint8_t data) {
    ...
    int sendCRLF = 2;
    if (data == 0) return;
    if (BreakKey && data == BreakKey) {      // if the user wants to stop the progran
...
bool notbefore(int i, uint8_t * current_keys, uint8_t * prev_keys) {
    ...
    for (int j = 0; j < 6; j++) {
        if (prev_keys[j] == test) return false;
    }
    return true;
}
static void process_kbd_report(hid_keyboard_report_t const * report, uint8_t n) {
