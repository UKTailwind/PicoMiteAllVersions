...
void hal_calendar_epoch_to_tm(time_t epoch, struct tm *out)
{
    ...
    uint64_t erayear = (uint64_t)((eraday - eraday / (DAYS_PER_4_YEARS - 1)
                                  + eraday / DAYS_PER_CENTURY
                                  - eraday / (DAYS_PER_ERA - 1)) / 365);          /* [0, 399]   */
    uint64_t yearday = eraday - (DAYS_PER_YEAR * erayear
                                 + erayear / 4 - erayear / 100);                  /* [0, 365]   */
...
time_t hal_calendar_tm_to_epoch(const struct tm *tm)
{
    ...
    return (time_t)(days * SECS_PER_DAY
                    + (int64_t)tm->tm_hour * SECS_PER_HOUR
                    + (int64_t)tm->tm_min  * SECS_PER_MIN
                    + (int64_t)tm->tm_sec);
}
