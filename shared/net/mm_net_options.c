...
int mm_net_mminfo(unsigned char * expr, int64_t * out_iret,
    ...
    case MM_NET_INFO_IP_ADDRESS: {
        char ipbuf[32];
        int rc = hooks && hooks->ip_address ? hooks->ip_address(ipbuf, sizeof ipbuf) : HAL_NET_ERR;
    ...
    case MM_NET_INFO_WIFI_STATUS:
        mm_net_info_set_int(out_iret, out_targ,
                            hooks && hooks->wifi_status ? hooks->wifi_status() : HAL_NET_UNSUPPORTED);
        return 1;
    case MM_NET_INFO_TCPIP_STATUS:
        mm_net_info_set_int(out_iret, out_targ,
                            hooks && hooks->tcpip_status ? hooks->tcpip_status() : HAL_NET_UNSUPPORTED);
        return 1;
